...
int main() {
    ...
    linuxBackEndInit(&linuxBackEnd, size);

    Renderer renderer;
    // rendererInit(&renderer, size, (LinuxBackEnd*)&linuxBackEnd);
    rendererInit(&renderer, size, &linuxBackEnd.backEnd);
    ...
    object.transform = mat4RotateZ(radians(180));
    t = mat4RotateZ(0);
